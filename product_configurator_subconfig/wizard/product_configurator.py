...
class ProductConfigurator(models.TransientModel):
    ...
    def _prefixes(self):
        ...
        res.update({"subattr_prefix": "__subproduct_attribute-"})
        return res
    ...
    def onchange(self, values, field_name, field_onchange):
        ...
        subattr_vals = {k: values[k] for k in values if k.startswith(subattr_prefix)}

        value_ids = [attr_id for attr_id in subattr_vals.values() if attr_id]

        domain = [("product_tmpl_id", "=", subproduct.id)]
        domain += [("attribute_value_ids", "=", val_id) for val_id in value_ids]
    ...
    @api.model
    def get_cfg_subproduct_fields(self, wiz, subproduct_line, default_attrs=None):
        if not default_attrs:
            default_attrs = {}
        ...
        for step in substeps:
            res["__substep_%d" % step.id] = dict(
                default_attrs, type="selection", string=step.config_step_id.name,
            )
        return res
    ...
    def fields_get(self, allfields=None, write_access=True, attributes=None):
        res = super(ProductConfigurator, self).fields_get(
            allfields=allfields, write_access=write_access, attributes=attributes
        )
    ...
    def add_dynamic_fields(self, res, dynamic_fields, wiz):
        ...
        subproduct_notebook = etree.Element(
            "notebook", colspan="4", name="subproduct_notebook",
        )
        ...
        active_subproduct = (
            subproduct and subproduct not in cfg_subproducts or subproduct_line.multi
        )

        if active_subproduct:
            subproduct_config_page = etree.Element(
                "page", name="subproduct_form", string="Subproduct Configuration",
            )
            subproduct_config_group = etree.Element(
                "group", name="subproduct_config_group",
            )
        ...
        subproducts_page = etree.Element(
            "page", name="subproducts", string="Subproducts",
        )
        subproduct_notebook.append(subproducts_page)

        subproduct_group = etree.Element("group", name="subproduct_group",)
    ...
    def action_next_step(self):
        ...
        active_step = self.env["product.config.step.line"].browse(int(self.state))
        ...
        sessions = self.child_ids.filtered(lambda s: s.product_tmpl_id == subproduct)
