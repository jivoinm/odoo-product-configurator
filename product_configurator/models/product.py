...
class ProductTemplate(models.Model):
    ...
    mako_tmpl_name = fields.Text(
        string="Variant name", help="Generate Name based on Mako Template", copy=True
    )
    ...
    def create_config_wizard(
        self, model_name="product.configurator", extra_vals=None, click_next=True
    ):
        ...
        action = {
            ...
            "view_mode": "form",
            "target": "new",
            "context": dict(self.env.context, wizard_model="product.configurator",),
        }
    ...
    def _get_possible_combinations(
        ...
        if not self.active:
            return _(
                "The product template is archived so no " "combination is possible."
            )
...
class ProductProduct(models.Model):
    ...
    weight_dummy = fields.Float(
        string="Manual Weight", digits=dp.get_precision("Stock Weight"),
    )
    weight = fields.Float(
