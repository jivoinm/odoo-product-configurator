...
class ProductConfigLine(models.Model):
    ...
    attr_line_val_ids = fields.Many2many(
        comodel_name="product.attribute.value",
        relation='product_template_attribute_line_value_rel',
...
class ProductConfigStepLine(models.Model):
    ...
    config_step_id = fields.Many2one(
        comodel_name="product.config.step", string="Configuration Step", required=True
    )
...
class ProductConfigSession(models.Model):
    ...
    def create_get_variant(self, value_ids=None, custom_vals=None):
        ...
        duplicates = self.search_variant(value_ids=value_ids, custom_vals=custom_vals)
    ...
    def get_variant_vals(self, value_ids=None, custom_vals=None, **kwargs):
        ...
        if custom_vals:
            vals.update({"value_custom_ids": self.encode_custom_values(custom_vals)})
        return vals

    def get_session_search_domain(
        self, product_tmpl_id, state="draft", parent_id=None, user_id=None
    ):
    ...
    def get_next_step(
        self, state, product_tmpl_id=False, value_ids=False, custom_value_ids=False
    ):
        ...
        if not cfg_step_lines:
            if (value_ids or custom_value_ids) and not state == "select":
                return False
            elif not (value_ids or custom_value_ids) and not state == "select":
                raise UserError(
                    _(
    ...
    def get_variant_search_domain(
        ...
        domain = [("product_tmpl_id", "=", product_tmpl_id), ("config_ok", "=", True)]
    ...
    def validate_configuration(
        self, value_ids=None, custom_vals=None, product_tmpl_id=False, final=True
    ):
        ...
        if set(value_ids) - set(avail_val_ids):
            ...
            for attr, val in group_by_attr.items():
                message += "\n%s: %s" % (attr.name, ", ".join(val.mapped("name")))
    ...
    @api.model
    def search_variant(self, value_ids=None, custom_vals=None, product_tmpl_id=None):
    ...
    def search_session(self, product_tmpl_id, parent_id=None, user_id=None):
        domain = self.get_session_search_domain(
            product_tmpl_id=product_tmpl_id, parent_id=parent_id, user_id=user_id
        )
