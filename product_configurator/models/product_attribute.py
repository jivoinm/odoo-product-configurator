...
class ProductAttribute(models.Model):
    ...
    multi = fields.Boolean(
        string="Multi", help="Allow selection of multiple values for " "this attribute?"
    )
...
class ProductAttributeLine(models.Model):
    ...
    def _check_valid_attribute(self):
        if any(
            (not line.value_ids and not line.custom)
...
class ProductAttributeValue(models.Model):
    ...
    product_ids = fields.Many2many(comodel_name="product.product", copy=False)
...
class ProductAttributePrice(models.Model):
    ...
    weight_extra = fields.Float(
        string="Attribute Weight Extra", digits=dp.get_precision("Stock Weight")
    )
