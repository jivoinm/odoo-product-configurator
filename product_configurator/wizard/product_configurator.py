...
class ProductConfigurator(models.TransientModel):
    ...
    def get_onchange_domains(
        self, values, cfg_val_ids, product_tmpl_id=False, config_session_id=False
    ):
    ...
    def _onchange_state(self):
        if self.config_session_id:
            self.config_session_id.write(
                {"value_ids": [[6, 0, self.value_ids.ids]], "config_step": self.state,}
            )
    ...
    def create(self, vals):
        ...
        vals.update(
            {"user_id": self.env.uid, "config_session_id": session.id,}
        )
    ...
    config_step_line_ids on the product.template."""
        ...
        wizard_action = {
            ...
            "view_mode": "form",
            "context": dict(self.env.context, wizard_id=self.id, view_cache=False,),
            "target": "new",
            "res_id": self.id,
        }
    ...
    def get_wizard_action(self, view_cache=False, wizard=None):
        ...
        wizard_action = {
            ...
            "view_mode": "form",
            "context": dict(
                wizard.env.context, wizard_id=wizard.id, view_cache=view_cache,
            ),
            "target": "new",
            "res_id": wizard.id,
        }
